...
export function SystemNetwork() {
  ...
  return (
    <section className="py-32 w-full min-h-screen bg-[#0A0B0F] relative overflow-hidden flex flex-col items-center">
      ...
      <div className="relative w-full max-w-6xl aspect-[16/9] md:aspect-[21/9] overflow-hidden group">
        {/* Animated Background Grid - More subtle and integrated */}
        ...
        {/* Connection Lines (SVG) with animated dashes */}
          ...
          {nodes.map((node) => (
            ...
            >
                ...
                >
                    ...
                    <div 
                        className={clsx(
                        "w-12 h-12 flex items-center justify-center transition-all duration-300 clip-path-hexagon",
                        activeNode === node.id 
                    ...
                    >
                        <div className={clsx(
                            ...
                            activeNode === node.id ? "bg-red-500 shadow-[0_0_15px_#ef4444]" : "bg-white/30"
                        )} />
                    </div>
                    ...
                    <div className={clsx(
                        "absolute top-14 left-1/2 -translate-x-1/2 text-[10px] font-mono tracking-widest uppercase whitespace-nowrap transition-colors duration-300 bg-black/50 px-2 py-1 rounded backdrop-blur-sm border border-white/5",
        ...
        {/* Info Panel HUD - Glassmorphic / HUD Style */}
        <motion.div 
            initial={false}
            animate={{ 
                x: activeNode ? 0 : "100%",
            ...
            className="absolute top-0 right-0 w-full md:w-96 h-full bg-[#0B0F1A]/80 backdrop-blur-2xl border-l border-white/5 p-8 flex flex-col justify-center z-20"
        >
            ...
            {activeNode && (
                <>
                    <div className="text-xs font-mono text-red-500 mb-2">ID: {nodes.find(n => n.id === activeNode)?.id.toUpperCase()}_01</div>
                    <h4 className="text-3xl text-white font-bold mb-6 tracking-tight uppercase">{nodes.find(n => n.id === activeNode)?.label}</h4>
                    
                    <p className="text-white/70 leading-relaxed font-light border-l-2 border-white/10 pl-4 mb-8">
                    ...
                    <button className="mt-12 w-full py-3 bg-white/5 border border-white/10 text-white text-xs tracking-widest uppercase hover:bg-white/10 transition-colors">
                        View Diagnostics
                    </button>
                </>
            )}
